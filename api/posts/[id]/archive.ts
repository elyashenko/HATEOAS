...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  console.log('=== Archive Handler Called ===');
  ...
  const url = `/api/posts/${id}/archive`;
  
  console.log('Forwarding to Fastify:', url);

  try {
    const method = (req.method || 'POST').toUpperCase() as 'GET' | 'POST' | 'PUT' | 'DELETE' | 'PATCH' | 'HEAD' | 'OPTIONS';
    ...
    // Для POST без тела не устанавливаем Content-Type
    ...
    console.log(`Archive response: ${statusCode}`);
